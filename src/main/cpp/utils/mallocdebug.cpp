...
void* myMalloc (size_t size, const char* file, int line, const char* func) {
	...
	LOGD("malloc of %zu = %p in %s line %d file %s", size, ret, func, line, file);
	return ret;
}

void* myRealloc (void* ptr, size_t size, const char* file, int line, const char* func) {
	void* ret = realloc(ptr, size);
	LOGD("realloc of %zu, %p = %p in %s line %d file %s", size, ptr, ret, func, line, file);
	return ret;
}
...
void* myMemset (void* ptr, int value, size_t num, const char* file, int line, const char* func) {
	LOGD("memset of %zu bytes at %p with %i in %s line %d file %s", num, ptr, value, func, line, file);
...
void* myMemcpy ( void * destination, const void * source, size_t num, const char* file, int line, const char* func) {
	LOGD("memcpy by %zu from %p - %p to %p - %p in %s line %d file %s", num, source, (char*)source + num, destination, (char*)destination + num, func, line, file);
	return memcpy(destination, source, num);
}

void myBcopy ( const void *src, const void *dest, size_t num, const char* file, int line, const char* func) {
	LOGD("memcpy by %zu from %p - %p to %p - %p in %s line %d file %s", num, src, (char*)src + num, dest, (char*)dest + num, func, line, file);
	// bcopy (src, dest, num);
}
