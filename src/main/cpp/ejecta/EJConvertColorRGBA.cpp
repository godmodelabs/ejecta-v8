...
static inline unsigned int ColorHashForString( const char *s, size_t length ) {
    unsigned int h = 0;
...
EJColorRGBA bufferToColorRBGA(const char *jsc, size_t length) {
    ...
    if( jsc[0] == '#' && length == 4 ) {
        ...
        // c.hex = c.components[0] + (c.components[1] << 8) + (c.components[2] << 16) + (c.components[3] << 24);
    }
        ...
        // c.hex = (c.rgba.a << 24) + (c.rgba.r << 16) + (c.rgba.g << 8) + c.rgba.g;
    }
